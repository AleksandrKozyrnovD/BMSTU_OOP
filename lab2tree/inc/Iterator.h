...
public:
	...
	explicit Iterator(const tree &other) noexcept;
	Iterator(const Iterator<Type> &iterator) noexcept;
	Iterator(const ConstIterator<Type>  &iterator) noexcept;
	...
	Iterator<Type> operator+(const size_t diff) const;
	Iterator<Type> operator-(const size_t diff) const;

	Iterator<Type> &operator+=(const size_t diff);
	Iterator<Type> &operator-=(const size_t diff);
